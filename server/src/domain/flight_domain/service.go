...
type NewFlightData struct {
	Plane              *plane_domain.Plane
	Seats              []*plane_domain.Seat
	FlightTag          string
...
func NewFlight(data *NewFlightData) (*Flight, []*FlightSeat, error) {
	...
	for i, seat := range data.Seats {
		...
		switch seat.Class() {
		case plane_domain.BusinessSeat:
			flightSeat, err = newFlightSeat(
				flight, seat, data.BusinessClassPrice,
			)

		case plane_domain.EconomySeat:
			flightSeat, err = newFlightSeat(
				flight, seat, data.EconomyClassPrice,
			)

		default:
			return nil, nil, plane_domain.ErrUnknownSeatClass
		}
...
func newFlightSeat(flight *Flight, seat *plane_domain.Seat, price shared.Price) (*FlightSeat, error) {
	return &FlightSeat{
